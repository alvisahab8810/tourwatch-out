...
function DestCard({ d }) {
  const primary   = d.images?.economy || d.images?.deluxe || "/assets/images/n-destination/img1.webp";
  const secondary = d.images?.deluxe  || d.images?.economy || primary;
    ...
    <div className="new-desti-card">
      <div className="desti-img-wrapper">
        <div className="desti-img change-hover">
          <img src={primary} alt={d.title} />
        </div>
        <div className="desti-img dual-change-hover">
          <img src={secondary} alt={d.title} />
        </div>
      </div>
      <div className="p-4">
        <div className="header">
          ...
          <div className="share-area">
            <span className="duration-badge">{d.duration || "4N/5D"}</span>
            <Link href="#">
        ...
        <div className="location">
          ...
          <span>
            {locs.length > 0 ? locs.join(" • ") : (d.state || d.country || "")}
          </span>
        </div>
        ...
        <div className="price-section">
          ...
          <Link href={`/family/national-destination/${slug}`}>View Package</Link>
        </div>
      </div>
    </div>
  );
