      ...
      if (entry.persons && entry.persons.length > 0) {
        entry.persons.forEach((person, i) => {
          if (person.governmentId) {
            ...
            zip.file(`ID_${entry.fullName}_${i + 1}.png`, base64Data, { base64: true });
            count++;
          }
        });
      }
...
useEffect(() => {
  ...
  if (filterStartDate && filterEndDate) {
    filtered = filtered.filter((entry) => {
      ...
      return entryDate >= new Date(filterStartDate) && entryDate <= new Date(filterEndDate);
    });
  }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setFilteredFamilyData((prevData) => prevData.filter((entry) => entry._id !== id));
    } catch (error) {
  ...
  const handleUndo = (id) => setDeletedIds(deletedIds.filter((item) => item !== id));
        ...
        <table className="table table-bordered">
          ...
          <tbody>
            {filteredFamilyData.map((entry, index) => (
              <tr key={index} style={{ opacity: deletedIds.includes(entry._id) ? 0.5 : 1 }}>
                <td>{entry.fullName}</td>
                <td>{entry.contact}</td>
                <td>{entry.email}</td>
                <td>{entry.totalPersons}</td>
                <td>
                  {entry.persons?.length ? (
                    <button className="view-btn" onClick={() => setSelectedPerson(entry.persons)}>
                      View Details
                    </button>
                  ) : (
                    "No persons"
                ...
                <td>
                  {entry.kids?.length ? (
                    <button className="view-btn" onClick={() => setSelectedKids(entry.kids)}> {/* ✅ Fixed here */}
                      View Details
                    </button>
                  ) : (
                    "No kids"
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button type="button" className="btn-close" onClick={() => setSelectedPerson(null)}></button>
              </div>
              <div className="modal-body">
                {selectedPerson.map((person, i) => (
                  <div key={i} className="d-flex justify-content-between">
                    ...
                    {person.governmentId && (
                      <button className="view-btn" onClick={() => setSelectedIdImage(person.governmentId)}>
                        View ID
                      </button>
                    )}
                  </div>
        ...
        <div className="modal fade show" style={{ display: "block", backgroundColor: "rgba(0,0,0,0.5)" }}>
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title">Government ID</h5>
                <button type="button" className="btn-close" onClick={() => setSelectedIdImage(null)}></button>
              </div>
              <div className="modal-body text-center">
                <img src={selectedIdImage} alt="Uploaded ID" className="img-fluid" style={{ maxHeight: "400px" }} />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button className="btn-close" onClick={() => setSelectedKids(null)}></button>
              </div>
              <div className="modal-body">
