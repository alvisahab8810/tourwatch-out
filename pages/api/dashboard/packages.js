...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const packages = await Package.find({})
      .select("destination packageType packageSubtype packageName duration basePrice finalPrice priceType status popular destinationHighlights amenities featureImage metaTitle metaDescription metaKeywords faqs schemas createdAt updatedAt")
      .sort({ createdAt: -1 })
      .lean();
