                ...
                <div key={idx} style={{ border: "1px solid #e2e8f0", borderRadius: 10, padding: "12px 14px", marginBottom: 10, background: "#fafafa", position: "relative" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 10 }}>
                    ...
                    <div>
                      ...
                      <input style={S.input} placeholder="e.g. 1 queen bed" value={room.bedType || ""} onChange={e => setRoom(idx, "bedType", e.target.value)} />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
                      ...
                      <div onClick={() => openRoomGallery(idx)} style={{ width: 64, height: 64, border: "2px dashed #cbd5e1", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", background: "#f8fafc" }}>
                        <MdAdd size={20} color="#94a3b8" />
                      </div>
                    </div>
                  </div>
                  {idx > 0 && (
                    <button style={{ position: "absolute", top: 10, right: 10, background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: 16 }} onClick={() => removeRoom(idx)}>✕</button>
                  )}
                </div>
              ))}
            </>
            ...
            <>
              ...
              {form.vehicles.map((veh, idx) => (
                <div key={idx} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12, padding: "12px 14px", border: "1px solid #e2e8f0", borderRadius: 10, background: "#fafafa", position: "relative" }}>
                  ...
                  {idx > 0 && (
                    <button style={{ position: "absolute", top: 8, right: 8, background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: 16 }} onClick={() => removeVehicle(idx)}>✕</button>
                  )}
                </div>
              ))}
            </>
            ...
            <>
              ...
              {form.activities.map((act, idx) => (
                <div key={idx} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 12, padding: "12px 14px", border: "1px solid #e2e8f0", borderRadius: 10, background: "#fafafa", position: "relative" }}>
                  ...
                  {idx > 0 && (
                    <button style={{ position: "absolute", top: 8, right: 8, background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: 16 }} onClick={() => removeActivity(idx)}>✕</button>
                  )}
                </div>
              ))}
            </>
...
/* ══════════════════════════════════════════════════
   MAIN PAGE
══════════════════════════════════════════════════ */
export default function SPVendors() {
  ...
  function openAdd() { setEditData({ ...EMPTY_FORM, vendorTab: tab }); setModal(true); }
  function openEdit(v) {
    setEditData({
      ...EMPTY_FORM, ...v,
  ...
  return (
    <SPLayout active="Vendors" spData={spData} onLogout={handleLogout}>
      <Head><title>Vendors — Tourwatchout</title></Head>

      <div style={{ padding: "28px 24px" }}>
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 700, color: "#0f172a" }}>Vendors</h1>
        </div>
